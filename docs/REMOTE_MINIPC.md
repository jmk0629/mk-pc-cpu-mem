...
| 모드 | 실행 위치 | 설정 | 24h | 용도 |
|---|---|---|---|---|
| (A) 대상 PC에서 실행 | 미니 PC (systemd) | `hosts: [{host: ""}]` (미니 PC 위) | ✅ 미니 PC가 항상 켜짐 | 정식 운영 |
| (B) 현재 PC에서 원격 감시 | Mac 등 (SSH) | `hosts: [{host: keymedi1}]` | ⚠️ 현재 PC 켜진 동안만 | 설치 없이 점검/개발 |
| (C) 멀티호스트 — 둘 다 | Mac 등 (로컬+SSH) | `hosts: [{host: ""}, {host: keymedi1}]` | ⚠️ 현재 PC 켜진 동안만 | 한 모니터로 여러 대 |

모드 (B)/(C)는 미니 PC에 **아무것도 설치하지 않고** 현재 PC에서 `ssh keymedi1` 로 docker/systemd 를
들여다본다. (C)는 로컬 Mac + 원격 미니 PC를 **한 모니터/한 봇**으로 본다(§4.6). 진짜 24시간은 (A).
...
## 4.6 모드 (C) — 멀티호스트(한 모니터로 여러 대)

`hosts` 목록에 로컬+원격을 함께 넣으면 한 모니터/한 봇이 모두 감시한다.
...
구현(`config.py`/`monitor.py`):
- **`Target.host`**: 모든 타겟이 자기 호스트를 가진다(`""`=로컬). 측정/재시작은 `target.host` 로 분기.
- **`HostConfig`** + `config.hosts`: 호스트별 `system`(전체 감시)·`discovery`(자동 발견)·`targets`(명시).
  `hosts` 가 없으면 레거시(`remote`/`discovery`/`targets`)에서 단일 호스트를 합성(하위호환).
- **`Monitor._refresh_targets`**: 호스트별 system + 명시 + 발견 타겟을 `tid="host|name"` 키로 병합.
  상태 머신/콜백이 tid 로 호스트를 구분 → 같은 이름이 여러 호스트에 있어도 충돌 없음.
- **호스트별 메트릭**: `/status`·`/top`·시스템 경보가 각 호스트를 따로 계산(`@host` 표기).
- 검증(라이브): 한 모니터가 `Mac [system]`(로컬) + `미니PC @keymedi1`·open-design·caddy 등(원격)을 동시 감시.
