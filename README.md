...
- `remote.host` — SSH 별칭 지정 시 현재 PC에서 그 PC를 원격 감시(미니 PC 설치 불필요). 빈 값=로컬
- `discovery.enabled` — 켜면 docker 컨테이너+systemd 서비스를 exclude만 빼고 자동 감시
- `targets[]` — 명시적 감시 대상 (system/docker/systemd/launchd/process)
